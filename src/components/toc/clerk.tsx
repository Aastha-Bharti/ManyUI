...
export function TOCItems({ ref, className, ...props }: ComponentProps<'div'>) {
  ...
  useEffect(() => {
    ...
    function onResize(): void {
      ...
      // Wavy params
      const amplitude = 8;     // how tall each wave crest is
      const wavelength = 24;   // how wide each wave is
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        w = Math.max(offset + wavelength, w);
        h = Math.max(h, bottom);

        // Build a wavy path segment
        const yStart = i === 0 ? top : null;
        const yEnd = bottom;

        // On first item, move to starting point
        if (i === 0) {
          d.push(`M ${offset} ${top}`);
        }

        // Create a "wave" from last position to the next
        const controlY = (i % 2 === 0) ? top - amplitude : top + amplitude;
        d.push(`Q ${offset + wavelength / 2} ${controlY}, ${offset} ${bottom}`);
      }
            ...
            maskImage: `url("data:image/svg+xml,${
              encodeURIComponent(
                `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${svg.width} ${svg.height}">
                  <path d="${svg.path}" stroke="black" stroke-width="2" fill="none" />
                </svg>`
              )
            }")`,
          }}
